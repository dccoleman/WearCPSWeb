...
if(ig.input.mouse.x>xs-w&&ig.input.mouse.x<xs+w&&ig.input.mouse.y>(ys-10)*this.scale&&ig.input.mouse.y<(ys+this.font.height*1.2)*this.scale){hoverItem=item;this.current=i;}
...
ig.baked=true;ig.module('game.menus.stats').requires('game.menus.base','game.xhr').defines(function(){MenuStats=Menu.extend({background:new ig.Image('media/background/stars.jpg'),scale:0.75,scroll:0,itemClasses:[MenuItemBack],init:function(){if(!window.Ejecta){this.itemClasses.shift();}
this.parent();this.current=1;this.width=ig.system.width/this.scale;this.items[0].y=(ig.system.height-130)/this.scale;if(this.items.length>1){this.items[1].y=(ig.system.height-80)/this.scale;}},draw:function(){var ctx=ig.system.context;ctx.save();ctx.scale(this.scale,this.scale);this.scroll+=ig.system.tick*10;this.scroll=this.scroll%this.background.height;this.background.draw(0,this.scroll-this.background.height);this.background.draw(0,this.scroll);ctx.restore();this.credits.draw(32,32);this.parent();}});StatsView=ig.Class.extend({data:null,workingIndicator:['','.','..','...'],font:new ig.Font('media/fonts/avenir-36-white.png'),timer:null,width:432,height:216,isExcerpt:false,totalGames:0,init:function(width,height){this.width=width;this.height=height;this.timer=new ig.Timer();},submit:function(data){if(window.Cocoon||window.Ejecta){var games=JSON.parse(localStorage.getItem('stats')||'[]');if(games.length>30){games.shift();}
...
ig.baked=true;ig.module('game.menus.pause').requires('game.menus.base').defines(function(){MenuItemSoundVolume=MenuItem.extend({getText:function(){return' -  '+(ig.soundManager.volume*100).round()+'%  + ';},left:function(){ig.soundManager.volume=(ig.soundManager.volume-0.1).limit(0,1);},right:function(){ig.soundManager.volume=(ig.soundManager.volume+0.1).limit(0,1);},click:function(){if(ig.input.mouse.x>240){this.right();}else{this.left();}}});MenuItemMusicVolume=MenuItem.extend({getText:function(){return' -  '+(ig.music.volume*100).round()+'%  + ';},left:function(){ig.music.volume=(ig.music.volume-0.1).limit(0,1);},right:function(){ig.music.volume=(ig.music.volume+0.1).limit(0,1);},click:function(){if(ig.input.mouse.x>240){this.right();}else{this.left();}}});MenuItemResume=MenuItem.extend({getText:function(){return'resume';},ok:function(){localStorage.setItem('soundVolume',ig.soundManager.volume);localStorage.setItem('musicVolume',ig.music.volume);ig.game.menu=null;}});MenuPause=Menu.extend({scale:0.8,clearColor:'rgba(0,0,0,0.8)',init:function(){this.parent();this.y=ig.system.height/3;this.items[0].y=230/this.scale;this.items[1].y=360/this.scale;this.items[2].y=500/this.scale;this.items[3].y=560/this.scale;},itemClasses:[MenuItemSoundVolume,MenuItemMusicVolume,MenuItemResume,MenuItemBack],draw:function(){this.width=ig.system.width/this.scale;var ctx=ig.system.context;this.parent();var s=0.85;ctx.save();ctx.scale(s,s);ctx.globalAlpha=0.5;this.font.draw('GAME PAUSED',ig.system.width/2/s,60/s,ig.Font.ALIGN.CENTER);ctx.restore();ctx.save();s=0.5;ctx.scale(s,s);ctx.globalAlpha=0.5;this.font.draw('SOUND',ig.system.width/2/s,200/s,ig.Font.ALIGN.CENTER);this.font.draw('MUSIC',ig.system.width/2/s,330/s,ig.Font.ALIGN.CENTER);ctx.restore();}});});

// lib/game/menus/detailed-stats.js
ig.baked=true;ig.module('game.menus.detailed-stats').requires('game.menus.base','game.xhr').defines(function(){DetailedStats=ig.Class.extend({container:null,closeButton:null,chart:null,maxScore:0,games:[],scoreScale:1,init:function(){this.container=ig.$('#stats');this.closeButton=ig.$('#stats-close');this.content=ig.$('#stats-content');this.canvas=ig.$('#stats-canvas');this.context=this.canvas.getContext('2d');},show:function(){this.container.style.height=ig.system.canvas.clientHeight+'px';this.container.style.display='block';this.content.style.display='none';var that=this;setTimeout(function(){that.container.className='expanded';setTimeout(function(){that.content.style.display='block';ig.xhr('backend/',{loadAll:true},that._initChart.bind(that));},400);},16);this.closeBound=this.close.bind(this);this.drawBound=this.draw.bind(this);this.closeButton.addEventListener('click',this.closeBound);this.shareFacebookBound=this.shareFacebook.bind(this);ig.$('#stats-share-facebook').addEventListener('click',this.shareFacebookBound);this.shareTwitterBound=this.shareTwitter.bind(this);ig.$('#stats-share-twitter').addEventListener('click',this.shareTwitterBound);},_initChart:function(response){this.resize();for(var i=0;i<response.games.length;i++){var g=response.games[i];g.created=new Date(g.created*1000);this.maxScore=Math.max(this.maxScore,g.score);}
this.scoreScale=this.roundToMagnitude(this.maxScore);this.timer=new ig.Timer();this.data=response.games;ig.system.stopRunLoop();this.draw();this.canvas.addEventListener('mousemove',this.drawBound);},close:function(){ig.system.startRunLoop();ig.$('#stats-share-facebook').removeEventListener('click',this.shareFacebookBound);ig.$('#stats-share-twitter').removeEventListener('click',this.shareTwitterBound);this.closeButton.removeEventListener('click',this.closeBound);this.canvas.removeEventListener('mousemove',this.drawBound);this.container.className='';this.content.style.display='none';var that=this;setTimeout(function(){that.container.style.display='none';},300);},resize:function(){var nw=this.content.clientWidth,nh=this.content.clientHeight-64;if(this.canvas.width!==nw||this.canvas.height!==nh){this.canvas.width=nw;this.canvas.height=nh;}},roundToMagnitude:function(n){var mag=this.magnitude(n);return Math.ceil((n*10)/mag)*mag/10;},magnitude:function(n){return Math.pow(10,Math.floor(Math.log(n)/Math.LN10));},draw:function(ev){this.resize();var mx=-100,my=-100;if(ev){var pos=this.canvas.getBoundingClientRect();mx=ev.clientX-pos.left;my=ev.clientY-pos.top;}
...
ig.baked=true;ig.module('game.menus.title').requires('game.menus.base','game.menus.detailed-stats','game.ease').defines(function(){MenuItemNormalMode=MenuItem.extend({getText:function(){return'new game';},ok:function(){ig.game.setGame();},});MenuItemRemoveAds=MenuItem.extend({getText:function(){return'remove ads';},ok:function(){ig.game.purchaseRemoveAds();}});MenuItemGameCenter=MenuItem.extend({getText:function(){return'gamecenter';},ok:function(){ig.game.gameCenter.showLeaderboard('score');}});MenuItemStats=MenuItem.extend({getText:function(){return'stats';},ok:function(){ig.game.detailedStats=ig.game.detailedStats||new DetailedStats();ig.game.detailedStats.show();ig.input.delayedKeyup.click=true;}});MenuItemAppStore=MenuItem.extend({getText:function(){return'download as app';},ok:function(){window.location.href=ig.ua.iOS?'https://goo.gl/qSntlx':'https://goo.gl/cg7rmI';}});MenuTitle=Menu.extend({itemClasses:[MenuItemNormalMode,MenuItemRemoveAds,MenuItemGameCenter],scale:0.75,y:0,init:function(){this.itemClasses=[];this.itemClasses.push(MenuItemNormalMode);if(window.Ejecta&&!localStorage.getItem('removeAds')){this.itemClasses.push(MenuItemRemoveAds);}
...
ig.baked=true;ig.module('game.entities.enemy').requires('impact.entity','impact.font','game.entities.explosion').defines(function(){EntityEnemy=ig.Entity.extend({word:'none',remainingWord:'none',health:8,currentLetter:0,targeted:false,font:new ig.Font('media/fonts/avenir-22-white.png'),fontActive:new ig.Font('media/fonts/avenir-22-orange.png'),speed:10,friction:{x:100,y:100},hitTimer:null,dead:false,angle:0,wordLength:{min:8,max:8},numKillParticles:20,numHitParticles:5,soundHit:new ig.Sound('media/sounds/hit.ogg'),reticleTimer:null,reticle:new ig.Image('media/ui/reticle.png'),type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,init:function(x,y,settings){this.parent(x,y,settings);this.font.letterSpacing=-1;this.fontActive.letterSpacing=-1;this.health=Math.random().map(0,1,this.wordLength.min,this.wordLength.max).round();this.word=this.getWordWithLength(this.health);this.remainingWord=this.word;this.hitTimer=new ig.Timer(0);this.dieTimer=new ig.Timer(0);ig.game.registerTarget(this.word.charAt(0),this);this.angle=this.angleTo(ig.game.player);if(!ig.ua.mobile){this.speed*=ig.game.speedFactor;}},getWordWithLength:function(l){var w='wtf';for(var i=0;i<20;i++){if(l>=2&&l<=12){w=ig.game.wordlist[l].random();}
else{w=String.fromCharCode('a'.charCodeAt(0)+(Math.random()*26).floor());}
if(!ig.game.targets[w.charAt(0).toLowerCase()].length){return w;}}
return w;},target:function(){this.targeted=true;ig.game.currentTarget=this;ig.game.unregisterTarget(this.remainingWord.charAt(0),this);ig.game.entities.erase(this);ig.game.entities.push(this);this.reticleTimer=new ig.Timer(0.5);},draw:function(){this.parent();if(this.reticleTimer){var d=this.reticleTimer.delta().map(-0.5,0,0,1);var ctx=ig.system.context;ctx.save();var scale=(1-d)*(1-d)*4;ctx.translate(this.pos.x+this.size.x/2-ig.game._rscreen.x,this.pos.y+this.size.y/2-ig.game._rscreen.y);ctx.scale(scale,scale);ctx.globalAlpha=Math.sqrt(d);ctx.rotate(d*Math.PI*1);this.reticle.draw(-128,-128);ctx.globalAlpha=1;ig.system.context.restore();}},drawLabel:function(){if(!this.remainingWord.length){return;}
var w=this.font.widthForString(this.word);var x=(this.pos.x-6).limit(w+2,ig.system.width-1)+ig.game._rscreen.x;var y=(this.pos.y+this.size.y-10).limit(2,ig.system.height-19)+ig.game._rscreen.y;var bx=ig.system.getDrawPos(x-w-2);var by=ig.system.getDrawPos(y-1);ig.system.context.fillStyle='rgba(0,0,0,0.75)';ig.system.context.fillRect(bx,by+5,w+8,24);if(this.targeted){this.fontActive.draw(this.remainingWord,x+2,y,ig.Font.ALIGN.RIGHT);}
else{this.font.draw(this.remainingWord,x+2,y,ig.Font.ALIGN.RIGHT);}},kill:function(silent){if(this.remainingWord.length){ig.game.unregisterTarget(this.remainingWord.charAt(0),this);}
...
ig.baked=true;ig.module('game.keyboard').requires('impact.font').defines(function(){ig.Keyboard=ig.Class.extend({background:new ig.Image('media/ui/keyboard.png'),empButton:new ig.Image('media/ui/emp-buttons.png'),pauseButton:new ig.Image('media/ui/pause.png'),font:new ig.Font('media/fonts/avenir-36-blue.png'),hoverImages:{leftEdge:new ig.Image('media/ui/key-edge-left.png'),rightEdge:new ig.Image('media/ui/key-edge-right.png'),normal:new ig.Image('media/ui/key.png')},multiBar:new ig.Image('media/ui/bar-blue.png'),multiIndicator:new ig.Image('media/ui/multi-indicator.png'),multiSounds:{2:new ig.Sound('media/sounds/multi-2.ogg'),3:new ig.Sound('media/sounds/multi-3.ogg')},width:0,height:0,hoverKey:null,expectedKeys:null,keys:[['q','w','e','r','t','y','u','i','o','p'],['a','s','d','f','g','h','j','k','l'],['z','x','c','v','b','n','m']],layout:{key:{width:64,height:106}},init:function(callback){ig.system.canvas.addEventListener('touchmove',this.touchmove.bind(this),false);ig.system.canvas.addEventListener('touchend',this.touchend.bind(this),false);ig.system.canvas.addEventListener('touchstart',this.touchstart.bind(this),false);this.callback=callback;this.width=this.background.width;this.height=(ig.ua.mobile?this.background.height:64);this.drawScale=ig.system.width/this.width;this.x=0;this.y=ig.system.height/this.drawScale-this.height;},getCurrentKey:function(touches){var touch=touches[touches.length-1];var documentScale=(parseInt(ig.system.canvas.offsetWidth)||ig.system.realWidth)/ig.system.width;var touchX=(touch.clientX/documentScale)/this.drawScale,touchY=(touch.clientY/documentScale)/this.drawScale;if(touch.clientX<64&&touch.clientY<64){return'ESC';}
else if(touchY<this.y-20){return'BACKSPACE';}
else if(touchY>this.y+214&&touchX>this.x+560){return'ENTER';}
var row=((touchY-this.y)/this.layout.key.height)|0;if(row<0||!this.keys[row]){return;}
var offsetX=(this.background.width-this.keys[row].length*this.layout.key.width)/2;var col=((touchX-this.x-offsetX)/this.layout.key.width)|0;var key=this.keys[row][col];if(this.expectedKeys){var closest=Infinity;for(var i=0;i<this.expectedKeys.length;i++){var kpos=ig.Keyboard.Map[this.expectedKeys[i]];var keyX=kpos[0]*this.layout.key.width+this.layout.key.width/2,keyY=kpos[1]*this.layout.key.height+this.layout.key.height/2;var dx=touchX-this.x-keyX,dy=touchY-this.y-keyY;var distance=Math.sqrt(dx*dx+dy*dy);if(distance<this.layout.key.width&&distance<closest){closest=distance;key=this.expectedKeys[i];}}}
return key;},touchstart:function(ev){var key=this.getCurrentKey(ev.touches);if(key!=='ESC'&&key!=='ENTER'&&key!=='BACKSPACE'){this.hoverKey=key;}},touchmove:function(ev){var key=this.getCurrentKey(ev.touches);if(key!=='ESC'&&key!=='ENTER'&&key!=='BACKSPACE'){this.hoverKey=key;}},touchend:function(ev){var key=this.getCurrentKey(ev.changedTouches);if(key){this.callback(key);}
this.hoverKey=null;},showMultiplier:function(m){this.multiplierTimer=new ig.Timer(2);this.multiplierIndex=m-2;this.multiSounds[m].play();},drawFull:function(){var ctx=ig.system.context;ctx.save();ctx.globalAlpha=1;ctx.scale(this.drawScale,this.drawScale);this.background.draw(this.x,this.y);this.drawMultiplierBar(this.x,this.y);if(this.hoverKey){var offsetX=0;var hoverImage=this.hoverImages.normal;if(this.hoverKey=='q'){hoverImage=this.hoverImages.leftEdge;offsetX=26;}
else if(this.hoverKey=='p'){hoverImage=this.hoverImages.rightEdge;offsetX=-26;}
var kpos=ig.Keyboard.Map[this.hoverKey];var x=(kpos[0]*this.layout.key.width-19+offsetX),y=this.y+kpos[1]*this.layout.key.height-112;ctx.globalAlpha=0.9;hoverImage.draw(x,y);ig.system.context.globalAlpha=1;this.font.draw(this.hoverKey.toUpperCase(),x+51,y+24,ig.Font.ALIGN.CENTER);}
if(!ig.game.emps){ctx.globalAlpha=0.7;}
this.empButton.drawTile(this.x+582,this.y+250,ig.game.emps,34,40);ctx.globalAlpha=0.15;this.pauseButton.draw(28,28);ctx.restore();},drawMultiplierBar:function(x,y){if(ig.game.streak>0){var multiLength=1-(75/(75+ig.game.streak*2));this.multiBar.draw(x,y,0,0,this.width*multiLength,2);}
var showMultiplierTime=this.multiplierTimer?this.multiplierTimer.delta():0;if(showMultiplierTime<0){ig.system.context.globalAlpha=showMultiplierTime.map(-2,0,2,0).limit(0,1);this.multiIndicator.drawTile(x+this.width*multiLength-30,y-22,this.multiplierIndex,32,18);ig.system.context.globalAlpha=1;}},drawMinimal:function(){var ctx=ig.system.context;ctx.save();ctx.globalAlpha=1;ctx.scale(this.drawScale,this.drawScale);this.drawMultiplierBar(this.x,this.y+62);if(!ig.game.emps){ctx.globalAlpha=0.7;}
this.empButton.drawTile(this.x+582,this.y,ig.game.emps,34,40);ctx.restore();},draw:function(){if(ig.ua.mobile){this.drawFull();}
else{this.drawMinimal();}}});ig.Keyboard.Map={q:[0,0],w:[1,0],e:[2,0],r:[3,0],t:[4,0],y:[5,0],u:[6,0],i:[7,0],o:[8,0],p:[9,0],a:[0.5,1],s:[1.5,1],d:[2.5,1],f:[3.5,1],g:[4.5,1],h:[5.5,1],j:[6.5,1],k:[7.5,1],l:[8.5,1],z:[1.5,2],x:[2.5,2],c:[3.5,2],v:[4.5,2],b:[5.5,2],n:[6.5,2],m:[7.5,2]};});
...
this.setTitle();this.wordlist=ig.WORDS.EN;},reset:function(){this.entities=[];this.currentTarget=null;this.wave=ig.copy(ZType.WAVES[this.difficulty]);var first='a'.charCodeAt(0),last='z'.charCodeAt(0);this.targets={'ä':[],'ü':[],'ö':[],'ß':[]};for(var i=first;i<=last;i++){this.targets[String.fromCharCode(i)]=[];}
this.score=0;this.rs=0;this.streak=0;this.longestStreak=0;this.hits=0;this.misses=0;this.kills=0;this.multiplier=1;this.gameTime=0;this.isPersonalBest=false;this.speedFactor=1;this.lastKillTimer=new ig.Timer();this.spawnTimer=new ig.Timer();this.idleTimer=new ig.Timer();this.waveEndTimer=null;},nextWave:function(){this.wave.wave++;this.wave.spawnWait=(this.wave.spawnWait*0.97).limit(0.2,1);this.wave.spawn=[];this.speedFactor*=this.wave.speedIncrease;var dec=0;for(var t=0;t<this.wave.types.length;t++){var type=this.wave.types[t];type.count-=dec;if(this.wave.wave%type.incEvery==0){type.count++;dec++;}
for(var s=0;s<type.count;s++){this.wave.spawn.push(t);}}
this.wave.spawn.sort(function(){return Math.random()-0.5;});},spawnCurrentWave:function(){if(!this.wave.spawn.length){if(this.entities.length<=1&&!this.waveEndTimer){this.waveEndTimer=new ig.Timer(2);}
else if(this.waveEndTimer&&this.waveEndTimer.delta()>0){this.waveEndTimer=null;this.nextWave();}}
else if(this.spawnTimer.delta()>this.wave.spawnWait){this.spawnTimer.reset();var type=this.wave.types[this.wave.spawn.pop()].type;var x=Math.random().map(0,1,10,ig.system.width-10);var y=-30;this.spawnEntity(type,x,y,{healthBoost:this.wave.healthBoost},true);}},spawnEntity:function(type,x,y,settings,atBeginning){var ent=new(type)(x,y,settings||{});if(atBeginning){this.entities.unshift(ent);}
else{this.entities.push(ent);}
if(ent.name){this.namedEntities[ent.name]=ent;}
return ent;},registerTarget:function(letter,ent){var c=this.translateUmlaut(letter.toLowerCase());this.targets[c].push(ent);if(!this.currentTarget){this.setExpectedKeys();}},unregisterTarget:function(letter,ent){var c=this.translateUmlaut(letter.toLowerCase());this.targets[c].erase(ent);if(!this.currentTarget){this.setExpectedKeys();}},setExpectedKeys:function(){this.keyboard.expectedKeys=[];for(var k in this.targets){if(this.targets[k].length){this.keyboard.expectedKeys.push(k);}}},_umlautTable:{'ä':'a','ü':'u','ö':'o','ß':'s'},translateUmlaut:function(k){if(!ig.ua.mobile&&false){return k;}
return this._umlautTable[k]||k;},keypress:function(ev){if(ev.target.tagName=='INPUT'||ev.ctrlKey||ev.altKey||this.mode!=ZType.MODE.GAME||this.menu){return true;}
...
if(letter=='ESC'){this.menu=new MenuPause();return true;}
...
this.reset();this.mode=ZType.MODE.TITLE;this.menu=new MenuTitle();this.emps=0;},update:function(){if(ig.input.pressed('menu')){if(this.menu&&this.menu instanceof MenuPause){this.menu=null;}
else if(!this.menu){this.menu=new MenuPause();}}
...
this.parent();if(this.mode===ZType.MODE.GAME){this.spawnCurrentWave();}
...
ig.game.setTitle();});}});ZType.MODE={TITLE:0,GAME:1,GAME_OVER:2};ZType.MULTIPLIER_TIERS={20:2,50:3};ZType.WAVES={MOBILE:{wave:0,spawn:[],spawnWait:1,healthBoost:0,speedIncrease:1.01,types:[{type:EntityEnemyOppressor,count:0,incEvery:9},{type:EntityEnemyDestroyer,count:0,incEvery:4},{type:EntityEnemyMine,count:3,incEvery:1}]},DESKTOP:{wave:0,spawn:[],spawnWait:0.7,healthBoost:0,speedIncrease:1.05,types:[{type:EntityEnemyOppressor,count:0,incEvery:7},{type:EntityEnemyDestroyer,count:0,incEvery:3},{type:EntityEnemyMine,count:3,incEvery:1}]}};var canvas=document.getElementById('canvas');var width=480;var height=720;if(ig.ua.mobile){var resize=function(){height=Math.min((window.innerHeight/(window.innerWidth))*width,852);canvas.style.width=window.innerWidth+'px';canvas.style.height=(window.innerWidth/width)*height+'px';if(ig.game&&ig.system){ig.system.resize(width,height);}}
window.addEventListener('resize',function(){setTimeout(resize,500);});resize();}
ig.System.drawMode=ig.System.DRAW.SUBPIXEL;ig.Sound.use=[ig.Sound.FORMAT.OGG,ig.Sound.FORMAT.MP3];var initApp=function(){var loaderClass=window.Ejecta?ig.SilentLoader:ig.RiseLoader;ig.main('#canvas',ZType,60,width,height,1,loaderClass);};if(window.device){document.addEventListener('deviceready',initApp,false);}else{initApp();}
if(!window.Ejecta&&!window.device&&!window.Cocoon){ig.$('body')[0].className=ig.ua.mobile?'mobile':'desktop';var closeButton=ig.$('#gsense-close');if(closeButton&&ig.ua.mobile){setTimeout(function(){closeButton.style.display='block';closeButton.addEventListener('touchend',function(){setTimeout(function(){ig.$('#canvas').style.display='block';ig.$('#gsense').style.display='none';},250);closeButton.style.display='none';});},1000);}
else{ig.$('#canvas').style.display='block';}}});